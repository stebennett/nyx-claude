...
- **A failing deliver check never re-dispatches `card-deliverer`.** It has **no rework mode** — its only terminal action is `gh pr create`, and the PR already exists (re-running it errors). Once its PR is open, `card-deliverer` is done with this card. Route each blocking finding by **what can actually fix it, and which PR it's on**:
  ...
  - **Design PR** — never `card-implementer`; its doctrine is code-first TDD and the branch is docs-only:
    - **`DLV-DOCS` (an expected phase doc missing from the branch) → you persist it yourself.** You are its sole writer and you still hold it: commit the missing doc to the design branch and push, then **re-run the check**. No agent, no rework, no budget — a missing phase doc here is your own persistence bug, not something to dispatch an agent at.
    - **`DLV-BASE` / `DLV-BODY-TRUE` where the body merely overclaims → you fix it yourself**, exactly as on the implementation PR (`{gh_command} pr edit`), then re-run the check.
    ...
    - Re-dispatch **`card-designer`** in rework mode — spending `reworks.deliver` — only for a finding about the design's *content itself* being wrong, never for a doc-plumbing or PR-metadata problem.
...
4. Apply the transition:
   ...
   - `verdict: fail` from **any checker** → **automatic rework of its producer**: if `reworks.<producer> < check_budget.<producer>`, increment it, delete the stale check doc (`<phase>-check.md`, or `deliver-check-design.md` on a design PR), and re-dispatch that producer in rework mode with the checker's blocking findings verbatim — **slice → `card-slicer`; design → `card-designer`. Deliver → *not* `card-deliverer`** (it has no rework mode and is never re-dispatched once its PR is open); route by finding **and by which PR it's on**. On the **implementation PR**: `DLV-BASE` and an overclaiming `DLV-BODY-TRUE` **you fix yourself** on the open PR (`{gh_command} pr edit`), spending no budget, then re-run the check; every finding needing a **commit on the branch** (`DLV-DOCS`, `DLV-PURITY`, a `DLV-BODY-TRUE` whose code is genuinely absent) goes to **`card-implementer`** in rework mode and spends `reworks.deliver`. On the **design PR** — never `card-implementer`, whose doctrine is code-first TDD against a docs-only branch: `DLV-DOCS` (a phase doc missing from the branch) is **your own persistence bug** as the docs' sole writer — commit it to the design branch and push yourself, then re-run the check, spending nothing; `DLV-BASE`/an overclaiming `DLV-BODY-TRUE` you fix yourself exactly as above; `DLV-PURITY` (code on a docs-only design branch) should be impossible — never auto-repair it, `status: blocked` and surface it; and `card-designer` is re-dispatched, spending `reworks.deliver`, only for a finding about the design's *content itself*, never a doc-plumbing or PR-metadata problem. Else `status: blocked` with the blocker **`check failed — <failing criterion ids>`** (e.g. `check failed — DSG-AC-COVERED, DSG-SCOPE`), and the driver decides. **Advisory findings never trigger rework** — they are recorded in the check doc and ride the PR.
