...
The line is a triage index, not a substitute for the docs. **Open a card's full phase docs only when its line flags something** — any `reworks > 0`, an est/actual divergence, `slices > 1`, `human-comments > 0`, or a blocked episode; a card clean on every field is covered with no deep read. For a flagged card, read:
...
- **The check docs:** every `*-check*.md` and `split-acceptance.md` for the card (per-slice `deliver-check-<k>.md` included) — **glob, never hardcode**. Each opens with YAML frontmatter `verdict:` + a `criteria: {ID: pass|fail|na}` map: **grep those maps for the by-criterion tally (§2), not the whole doc** — open a body only when a verdict needs its evidence. `SPL-*` ids feed that tally like `SLC-*`/`DSG-*`/`DLV-*`.
