...
2. **Both PRs'** comment threads — the design PR (`design_pr_url`) and the implementation PR (`pr_url`): `{gh_command} pr view <url> --json comments,reviews,state` plus `{gh_command} api repos/{owner}/{repo}/pulls/{n}/comments` (reactions included; both urls stay on done cards — for legacy cards missing them, recover PR numbers from merge commit subjects). From these extract: (a) which panel comments earned a 👍 and which were ignored, per lens; (b) panel comments the human **pushed back on** in replies — direct evidence of a miscalibrated lens; (c) the human's **own** review comments — each one is something no agent caught — and, per comment, whether the orchestrator addressed it (`[kanban] Addressed in <commit-url>`) or replied `[kanban] Not actioned — <reason>` (a `Not actioned` reply is a human instruction the machine could not carry out — a gap); (d) everything on the **design PR** — that's feedback on the design itself, aimed at the designer's prompt, and design-PR merge latency vs comment volume shows whether designs arrive review-ready; (e) human review verdicts, the review-complete signal (a submitted review or a `REVIEWED` comment), and any PR closed unmerged.
