...
- Add `knowledge` entries for any reusable convention (scope: repo). **Significant architecture/technology decisions become `proposed_adrs`** (check `docs/adrs/README.md` first; supersede rather than duplicate) — the design phase is where most ADRs are born, and the orchestrator persists them once the design **check** passes (`card-design-checker`'s `verdict: pass`), before the gate and before the design PR opens. It holds them unwritten until then: writing an ADR the checker may still reject would reserve a number against a decision that gets reworked. A `fail` sends the design back to you, and you propose again.
