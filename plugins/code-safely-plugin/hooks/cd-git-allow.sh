...
# Refuse anything beyond a single `cd <path> && git <readonly>`: no pipes, extra
# chaining, substitution, or redirection may ride along (e.g. cd x && git log | bash).
if printf '%s' "$COMMAND" | grep -qE '\$\(|`|>|;|\|'; then
    exit 0
fi
# Exactly one '&&' (i.e. exactly two '&' chars) — nothing else may chain on.
if [ "$(printf '%s' "$COMMAND" | tr -cd '&' | wc -c | tr -d ' ')" != "2" ]; then
    exit 0
fi

# Extract the git subcommand
GIT_CMD=$(printf '%s' "$COMMAND" | sed -nE 's/.*&&[[:space:]]*git[[:space:]]+([^[:space:]]+).*/\1/p')
