...
#      are unavailable
#   3. Saves current % to ${TMPDIR:-/tmp}/cc-context-<uid>/pct
#      (per-user private dir; other scripts can read this)
...
# Per-user private scratch dir. Avoids predictable, world-writable /tmp paths
# (symlink/TOCTOU risk on shared hosts). Namespaced by UID, created mode 0700.
CC_TMPDIR="${TMPDIR:-/tmp}/cc-context-$(id -u)"
mkdir -p "$CC_TMPDIR" 2>/dev/null && chmod 700 "$CC_TMPDIR" 2>/dev/null
STATE_FILE="$CC_TMPDIR/state"
PCT_FILE="$CC_TMPDIR/pct"
COUNTER_FILE="$CC_TMPDIR/count"
...
get_context_pct() {
    ...
    local latest
    # Portable newest-file selection (BSD/GNU): ls -t sorts by mtime, no find -printf
...
EVAC_COOLDOWN_FILE="$CC_TMPDIR/evac-last"
