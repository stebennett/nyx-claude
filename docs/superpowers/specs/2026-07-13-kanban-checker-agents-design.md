...
**Unchanged:** `card-slicer`, `card-designer`, `card-implementer`, `card-tester`, `card-deliverer`.
...
A missing key, or a missing producer within one, defaults to **`on`** and to a budget of **2**
...
2. Add `checks:` and `check_budget:` to `config.md` with the defaults above.
